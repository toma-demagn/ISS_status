...
function SatelliteMap({ data }) {
  ...
  let coordinates = TLEData
    ? TLEData.coordinates.map(([lng, lat]) => [lat, lng])
    : [];
  ...
  if (
    longitude < 0 &&
    Date.now() - updated_at > (180 * ISS_ORBIT_TIME_MS) / 2 / (longitude + 180)
  ) {
                  ...
                  {illuminations && illuminations.length > 0 ? (
                    <>
                      ...
                      {illuminations.map((illumination, index) => (
                        <pre key={index}>
                          {formatIllumination(illumination)}
                        </pre>
                      ))}
                    </>
                  ) : (
